...
class OntologyTerm:
    ...
    def get_db_refs(self, as_str=False):
        if self.__dbRefs is None:
            return None
        return "//".join(self.__dbRefs) if as_str else self.__dbRefs

    def get_synonyms(self, as_str=False):
        if self.__synonyms is None:
            return None
        return "//".join(self.__synonyms) if as_str else self.__synonyms

    def is_a(self, parse=False, as_str=False):
        ...
        return "//".join(self.__subclassOf) if as_str else self.__subclassOf
    ...
    def set_term(self, term: str):
        ...
        if (
            term.startswith("obsolete") and term != "obsolete class"
        ) or term.startswith("deprecated"):
            self.set_is_obsolete()
    ...
    def __str__(self):
        ...
        return list_to_string(values, null_str="NULL", delim="\t")
            ...
            if propType is not None:
                if propType == "db_ref":
                    ...
                    if propType in self.__annotationProperties:
                        self.__annotationProperties[propType] = (
    ...
    if len(relationships) == 1:
        ...
        if pType == "CLASS":
            ...
            raise NotImplementedError(
                "Issue parsing relationship - case not caught: " + relStr
            )
