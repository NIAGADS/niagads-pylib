...
class CacheManager:
    ...
    def __init__(
        ...
        self.__cache = RedisCache(
            serializer=serializer.value(), **config
        )  # need to instantiat the serializer
    ...
    def __get_db_url(self, db: Databases = None):
        match db:
            case Databases.GENOMICS:
                return get_settings().GENOMICSDB_URL.replace(
                    "postgresql:", "postgresql+asyncpg:"
                )
            case Databases.CACHE:
                return get_settings().API_CACHEDB_URL
            case Databases.METADATA:
                return get_settings().API_APPDB_URL.replace(
                    "postgresql:", "postgresql+asyncpg:"
                )
            case _:
                raise RuntimeError(
                    "Need to specify endpoint database - one of: genomics, cache, or metadata"
                )
