...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Look up a list of variants and retrieve annotation",
        allow_abbrev=False,
    ...
    parser.add_argument(
        "--pageSize", default=200, choices=[50, 200, 300, 400, 500], type=int
    )
    parser.add_argument(
        "--allConseqs",
        action="store_true",
    ...
    parser.add_argument(
        "--null_str",
        help="string for null values in .tab format",
        choices=["N/A", "NA", "NULL", ".", ""],
        default="",
    )
    ...
    try:
        ...
        variants.set_null_str(args.null_str)
        variants.set_page_size(args.pageSize)
        if args.allConseqs:
            variants.retrieve_full_annotation()
