...
class VEPJSONParser(object):
    ...
    def __init__(self, rankingFileName, rankConsequencesOnLoad=False, verbose=False, debug=False):
        self._debug = debug
        self._verbose = verbose
        self.logger = logging.getLogger(__name__)
        
        self.__consequenceParser = ConsequenceParser(rankingFileName, rankOnLoad=rankConsequencesOnLoad, verbose=verbose)
    ...
    def __find_matching_term(self, terms):
        ...
        """
        try: 
            return self.__consequenceParser.find_matching_consequence(terms, failOnMissing=True)
    ...
    def __verify_annotation(self):
        ...
        assert self.__annotation is not None, \
    ...
    def get_added_conseq_summary(self):
        ...
        if self.__consequenceParser.new_consequences_added():
            summary = ' '.join(("Added", xstr(self.__consequenceParser.get_new_conseq_count()),
                       "new consequences:", '[' + '; '.join(self.__consequenceParser.get_added_consequences()) +']'))
        return summary
    ...
    def get_frequencies(self, matchingVariantId=None):
        ...
        if len(cv) > 1:
            ...
            if fCount > 1: 
                ...
                if self._verbose:
                    self.logger.warn("Variant " + inputVariant + " mapped to multiple refSNPs/frequencies based on location not alleles")
            # else:
