...
def initialize_biosource_validator(metadataType: str, idField: str):
    ...
    metadataFile = path.join(
        args.metadataFileDir, f"{args.filePrefix}{metadataType}.{args.fileType}"
    )
...
def validate_participant_info():
    ...
    """
    participantValidator, participantIds = initialize_biosource_validator(
...
def validate_sample_info(expectedParticipantIds: List[str]):
    ...
    """
    sampleValidator, sampleIds = initialize_biosource_validator(
    ...
    if len(invalidSampleParticipants) > 0:
        error = ValidationError(
            f'Invalid participants found in the sample file: {list_to_string(invalidSampleParticipants, delim=", ")}'
        )
        if args.failAtFirst:
            raise error
        else:
...
if __name__ == "__main__":
    argParser = argparse.ArgumentParser(
        description="EXCEL to JSON w/Validation test", allow_abbrev=False
    )
    argParser.add_argument(
        "--metadataFileDir",
        help="full path to directory containing metadata files",
        required=True,
    )
    argParser.add_argument(
        "--schemaDir",
    ...
    argParser.add_argument(
        "--filePrefix", help="prefix to add to file names", default=""
    )
    argParser.add_argument(
        "--fileType",
        choices=["xlsx", "tab"],
        help="file type; assuming `file type == fil extension`",
        default="tab",
    )
    argParser.add_argument(
        "--logFile",
        help="log file name (full path).  Default log saves to current working directory",
        default="validation_test.log",
    )
    argParser.add_argument(
        "--failAtFirst",
        help="fail on first error; otherwise generatese a list of errors",
        action="store_true",
    )
