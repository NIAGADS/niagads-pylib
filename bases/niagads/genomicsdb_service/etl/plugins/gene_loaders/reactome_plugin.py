...
class ReactomeLoaderParams(BasePluginParams, PathValidatorMixin):
...
class ReactomeLoaderPlugin(AbstractBasePlugin):
    ...
    def extract(self):
        ...
        self.logger.debug(f"Successfully parsed {len(df.columns)} columns from file. Columns: {list(df.columns)}")
        
        # Filter for Homo sapiens
        filtered_df = df[df['species'] == 'Homo sapiens']
        self.logger.debug(f"Filtered by species 'Homo sapiens', remaining rows: {len(filtered_df)}") 
        
        # Filter for Ensembl gene IDs (starting with "ENSG")
        filtered_df = filtered_df[filtered_df['gene_id'].str.startswith('ENSG', na=False)]
        self.logger.debug(f"Filtered by gene ID prefix 'ENSG', remaining rows: {len(filtered_df)}")
    ...
    async def load(self, transformed: List[dict], mode):
        ...
        """
        self.logger.debug(f"Starting load with {len(transformed)} records in mode: {mode}")
