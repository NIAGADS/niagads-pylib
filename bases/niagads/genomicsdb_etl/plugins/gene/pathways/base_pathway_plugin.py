...
class PathwayMembershipLoaderPlugin(AbstractBasePlugin):
    ...
    async def _lookup_gene_primary_key(
        ...
        """check for gene primary key in stored hash."""
    ...
    async def _load_pathway(self, session, pathway_id: str, pathway_name: str):
        """
    ...
    async def _load_pathway_membership(
        ...
        is_multi_pathway_load: bool = False,
    ):
        """
        ...
        if not is_multi_pathway_load:
            # then you only need to lookup the pathway once
            pathway_pk = await self._load_pathway(
                session, annotations[0].pathway_id, annotations[0].pathway_name
            )

        memberships = []
        for record in annotations:
            # Lookup / possibly load pathway and get its primary key
            if is_multi_pathway_load:
                pathway_pk = await self._load_pathway(
