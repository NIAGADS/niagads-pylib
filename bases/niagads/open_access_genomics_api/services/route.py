...
class GenomicsRouteHelper(MetadataRouteHelperService):
    ...
    def __process_query_result(self, result, countsOnly: bool = False):
        if countsOnly and self.__query.counts_func is not None:
    ...
    async def __run_query(self, opts: QueryOptions):
        ...
        try:
            ...
            if self.__query.fetch_one or opts.fetch_one or opts.counts_only:
                if self.__query.json_field:
                    result = result[0][self.__query.json_field]
                    if isinstance(result, list):
                        return self.__process_query_result(
                            [dict(item) for item in result], opts.counts_only
                        )
                    return self.__process_query_result([result], opts.counts_only)
                return self.__process_query_result([result[0]], opts.counts_only)
            else:
