...
from niagads.open_access_api_models.data.core import RowModel
...
class ResponseModel(SQLModel, BaseModel):
    ...
    def has_count_fields(self):
        ...
        if isinstance(self.response[0].model_extra, dict):
            return any(
                f.startswith("num_") for f in self.response[0].model_extra.keys()
            )
    ...
    def to_text(self, format: ResponseFormat, **kwargs):
        ...
        if isinstance(self.response, dict):
            ...
            responseStr += (
                "\t".join([xstr(v, nullStr=nullStr) for v in self.response.values()])
                + "\n"
            )
        else:
