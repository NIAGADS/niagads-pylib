...
from niagads.utils.string import xstr, to_numeric
...
class VCFEntryParser(object):
    ...
    def parse_entry(self, inputStr):
        ...
        try:
            if "info" in result:
                infoStr = result["info"].replace(
                    "\\x2c", ","
                )  # \x escape causes JSON parsing issues
                infoStr = infoStr.replace(
                    "\\x59", "/"
                )  # \x59 is a semi-colon, but b/c its a delimiter can't use
                infoStr = infoStr.replace(
                    "#", ":"
                )  # b/c using pound sign as COPY delimiter
                info = dict(
    ...
    def get_frequencies(self, allele):
        ...
        populationFrequencies = {
            pop.split(":")[0]: pop.split(":")[1] for pop in vcfGMAFs.split("|")
        }
        vcfFreqs = {
            pop: {"gmaf": to_numeric(freq.split(",")[altIndex])}
    ...
    def infer_variant_end_location(self, alt):
        ...
        annotator = VariantAnnotator(
            ...
            alt,
            self.get("chrom"),
            int(self.get("pos"), debug=self._debug),
        )
