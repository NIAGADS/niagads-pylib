...
BEGIN
    ...
    CREATE VERTEX TYPE IF NOT EXISTS Reference.Ontology.term (
        term_id CHARACTER VARYING(32) PRIMARY KEY,       -- e.g. GO:0006915
        term_iri CHARACTER VARYING(150),                      -- full uri
        term CHARACTER VARYING(512),                     -- the term
        label CHARACTER VARYING(512),                    -- a display term for applications
        ...
        run_id INTEGER NOT NULL -- references admin.etlrun run_id
    );
    

    CREATE EDGE TYPE IF NOT EXISTS Reference.Ontology.triple (
        predicate CHARACTER VARYING(32) NOT NULL,         -- must point to a term_id
        run_id INTEGER NOT NULL -- references admin.etlrun run_id
    ...
    CREATE VERTEX TYPE IF NOT EXISTS Reference.Ontology.ontology (
        external_database_id INTEGER PRIMARY KEY, -- references reference.externaldatabase 
        ontology CHARACTER VARYING(100),
        run_id INTEGER NOT NULL -- references admin.etlrun run_id
    );
