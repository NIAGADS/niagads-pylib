...
class VEPConsequenceParser(object):
    ...
    def save_ranking_file(self, fileName=None):
        ...
        if fileName is None:
            fileName = (
                self.__rankingFileName.split(".")[0]
                + "_"
        ...
        if verify_path(
            fileName
        ):  # file already exists / add versioning for each newly added conseq
    ...
    def __parse_ranking_file(self):
        ...
        with open(self.__rankingFileName, "r") as fh:
            ...
            for row in reader:
                conseq = alphabetize_string_list(
                    row["consequence"]
                )  # ensures unique keys
                if "rank" in row:
    ...
    def get_consequence_rank(self, conseq, failOnError=False):
        ...
        if conseq in self.__consequenceRankings:
            ...
            if failOnError:
                raise IndexError(
                    "Consequence " + conseq + " not found in ADSP rankings."
                )
            return None
    ...
    def __add_consequence(self, terms):
        ...
        if conseqStr in referenceConseqs:
            raise IndexError(
                "Attempted to add consequence combination "
                + conseqStr
    ...
    def __update_rankings(self, terms=None):
        ...
        conseqs = (
            self.__add_consequence(terms)
            if terms is not None
            else self.get_known_consequences()
        )
        ...
        for grp in ConseqGroup:
            ...
            members = grp.get_group_members(
                conseqs, requireSubset
            )  # extract conseqs belonging to current group
            if self._debug:
        ...
        if terms is not None and self.__saveOnAddConsequence:
            if self._verbose:
                warning(
                    "Added new consequence `" + ",".join(terms) + "`. Saving version:",
                    self.get_new_conseq_count(),
                )
    ...
    def __sort_consequences(self, conseqs, conseqGrp):
        ...
        grpRankingDict = (
            ConseqGroup.HIGH_IMPACT.toDict()
            if conseqGrp.name != "MODIFIER"
            else conseqGrp.toDict()
        )

        completeRankingDict = (
            ConseqGroup.get_complete_indexed_dict()
        )  # needed for non-exclusive grps
        if self._debug:
            warning("GRP Dict:", print_dict(grpRankingDict, pretty=True))

        indexedConseqs = []
        for c in conseqs:
            indexedConseqs.append(
                self.__calculate_ranking_indexes(c, grpRankingDict, completeRankingDict)
            )
        ...
        return [
            ",".join(sc[1]) for sc in sortedConseqs
        ]  # (alpha index, term) tuples; return term as str
    ...
    def __calculate_ranking_indexes(self, conseq, grpDict, refDict):
        ...
        * if terms are not present in the ranking dict in the original code,
        ...
        indexedConseq = self.__internal_consequence_sort(
            memberTerms + nonMemberTerms, indexes, returnStr=False
        )
        if self._debug:
    ...
    def __internal_consequence_sort(self, terms, rankings, returnStr=False):
        ...
        sortedDict = OrderedDict(
            sorted(rankingDict.items(), key=lambda kv: kv[1])
        )  # sort by values
        if returnStr:
