...
class ConseqGroup(Enum):
    ...
    MODIFIER = ['intron_variant', 'mature_miRNA_variant', 'non_coding_transcript_variant',
                ...
                'TF_binding_site_variant', 'regulatory_region_amplification', 'regulatory_region_variant', 'intergenic_variant'] # GRP 3
    ...
    def get_group_members(self, conseqs, requireSubset=True):
        ...
        if requireSubset: # all terms must be in this group for a consequence to be a member
            ...
            if self.name == 'HIGH_IMPACT': # need to ignore consequence combos that include NMD/non-coding
                return [x for x in conseqs if is_overlapping_list(x.split(','), self.value) \
                            and not is_overlapping_list(x.split(','), ConseqGroup.NON_CODING_TRANSCRIPT.value) \
