        ...
        if isinstance(candidate_obj, dict):
            if "data" in candidate_obj and "request" in candidate_obj:
                ...
                if isinstance(candidate_data, list) and isinstance(
                    ...
                    data: list[DynamicRecord] = [
                        DynamicRecord(**row) for row in candidate_data
                    ]
...
class CountResponse(BaseResponseModel):
    data: List[CountExtendedRecord]  # FIXME - may need to be Type[CountRecordModel]
